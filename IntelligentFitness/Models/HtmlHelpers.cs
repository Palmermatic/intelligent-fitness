...
namespace IntelligentFitness.Models
{
    ...
    public static class HtmlHelperExtensions
    {
      public static MvcHtmlString LinkToRemoveNestedForm(this HtmlHelper htmlHelper, string linkText, string container, string deleteElement) {
        ...
        public static MvcHtmlString AddLink<TModel>(this HtmlHelper<TModel> htmlHelper, string linkText, string containerElement, string counterElement, string collectionProperty, Type nestedType)
            {
                var ticks = DateTime.UtcNow.Ticks;
                var nestedObject = Activator.CreateInstance(nestedType);
                var partial = Html.EditorFor(nestedType).ToHtmlString().JsEncode();
